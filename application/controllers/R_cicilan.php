...
class R_cicilan extends CI_Controller
{
    ...
    public function kwitansiCicilanDenda($id, $invoice, $pembayaranke,$ke)
    {
        $pdf = new FPDF('l','mm','A5');
        $data = $this->Denda_model->get_invoice_profile($invoice);
        $cek = $this->History_pembayaran_model->get_by_id($id);
        ...
        $pdf->Cell(7,25,'1',1,0);
        $x_axis = $pdf->getx();
        $pdf->vcell(85,25,$x_axis,'bayar cicil denda ke '.$ke.' untuk cicilan pembayaran ke-'.$pembayaranke.' - '.$data->invoice);
        $pdf->Cell(25,25,'1',1,0);
        $pdf->Cell(25,25,$cek->total_bayar,1,0);
        $pdf->Cell(25,25,$cek->total_bayar,1,0);
        $pdf->setXY(127, 101);
        ...
        $pdf->setXY(20, 120);
        $pdf->Cell(50,6,'(KASIR)',0,0,'C');
        $pdf->Output('kwitansicicilandenda'.$invoice.$pembayaranke.'.pdf', 'D');
    }
    ...
    function get_history_denda($invoice,$pembayaranke)
    {
        $filter = 'bayar denda cicilan ke '.$pembayaranke;
        $data['historydenda'] = $this->History_pembayaran_model->getDataHistoryPembayaran($invoice, 'bayar denda', $filter);
